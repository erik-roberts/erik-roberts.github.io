$(function() {
	...
	//was +1
	var spaces 		= w_width/(nmb_albums+.5);
	var cnt			= 0;
	...
	//instert code for initial picture here
	});
	
	function spreadPictures(){
		//hide initial picture
		if(current != -1){
			hideCurrentPicture();
		}
		...
		//original value was -90px
		$albums.not($album).stop().animate({'bottom':'-120px'},300);
		$album.unbind('click');
		...
		$album.find('.content')
			...
			//hide nav
			$nav.stop().animate({'left':'-200px'},300);
			enableshow = true;
		});
	}
